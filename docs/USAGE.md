...
- **履歴の保持**: チャット履歴はアプリ実行中のみメモリ上に保持される（最大50メッセージ）
- **履歴のクリア**: メニューまたはキャラクター切替で現在の会話履歴をクリア
...
Qwen3-TTS を使用したキャラクター別音声合成機能です。

#### セットアップ

1. `pip install qwen-tts` でパッケージをインストール
2. `config.json` の `tts.enabled` を `true` に設定
3. サイドバーの音声タブでキャラクターの TTS を有効化

#### 動作モード

| モード | 説明 |
|--------|------|
| CustomVoice | 事前定義スピーカー（Vivian 等）を使用 |
| VoiceDesign | テキスト記述で声質を指定（例: 優しい女性の声） |
| Base | 参照音声ファイルによる音声クローン |

#### ト書き（演出指示）機能

LLMの応答に括弧付きのト書きが含まれている場合、Qwen3-TTS の instruct パラメータが自動調整されます。

| ト書き例 | 効果 |
|---------|------|
| （小声で）、（ささやき） | 小声でささやくように話す |
| （大声で）、（叫んで） | 大きく力強い声 |
| （怒って）、（キレて） | 怒りの感情 |
| （笑って）、（嬉しそうに） | 楽しげな声 |
| （泣いて）、（悲しそうに） | 悲しみの声 |
| （驚いて）、（びっくり） | 驚きの声 |
| （照れて）、（恥ずかしそうに） | 恥ずかしそうな声 |
| （慌てて）、（焦って） | 焦った声 |
